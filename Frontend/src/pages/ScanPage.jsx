...
const ScanPage = () => {
  ...
  const handleRunAnalysis = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/api/scan/${scanId}/staticscan`);
  ...
  const finding = findings[0]; // show first vulnerability for now
        ...
        <div className="analysis-container animate-slide-up">
          ...
          <div className="analysis-top-row">
            ...
            <div className="analysis-col">
              <label>Suggested Fix</label>
              <div className="code-block suggestion relative-container">
                <pre><code>{finding.ai?.fix || "Use secure coding practices."}</code></pre>
                <button
                  className={`copy-btn-mini ${copied ? "copied" : ""}`}
                  onClick={() => handleCopy(finding.ai?.fix || "")}
                >
          ...
          <div className="analysis-bottom-row">
            <div className="explanation-section">
              <label>Why this code?</label>
              <div className="explanation-content">
                <p>{finding.ai?.explanation}</p>
                <div className="cwe-meta">
                  Industry Reference: <strong>{finding.cwe || "CWE-798"}</strong>
                </div>
              </div>
            </div>
          </div>
